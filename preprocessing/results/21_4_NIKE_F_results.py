...
with open('21_4_NIKE_F.txt', 'r') as f:
    data = f.readlines()
...
# 필터링된 라인에서 숫자를 찾아 1183.52로 곱하기
for line in filtered_data:
    match = pattern.search(line)
    if match:
        result = round(1183.52 * float(match.group(1)), 2)
...
for line in pound_lines:
    ...
    if match_front:
        result = round(1594.98 * float(match_front.group(1)), 2)
        results.append(result)
    elif match_back:
        result = round(1594.98 * float(match_back.group(2)), 2)
        results.append(result)
...
for line in dollar_lines:
    ...
    if match_front:
        result = round(1183.52 * float(match_front.group(1)), 2)
        results.append(result)
    elif match_back:
        result = round(1183.52 * float(match_back.group(2)), 2)
        results.append(result)
...
for line in euro_lines:
    ...
    if match_front:
        result = round(1352.63 * float(match_front.group(1)), 2)
        results.append(result)
    elif match_back:
        result = round(1352.63 * float(match_back.group(2)), 2)
        results.append(result)
...
# 결과를 txt 파일로 저장
with open('21_4_NIKE_F_results.txt', 'w') as f:
    for result in results:
        f.write(str(result) + '\n')