...
with open('/content/drive/MyDrive/BDP/ForeignData/2021_NIKE_F.txt', 'r') as f:
    lines = f.readlines()
    ...
    for line in lines:
        ...
        if len(items) < 3:  # 콤마가 2개 미만으로 나뉘는 경우, 해당 라인 제외
            removed_lines += 1
            continue
        first = items[0].replace(" ", "")  # first 부분의 모든 공백 제거
        middle = items[1]  # middle 부분의 공백을 제거하지 않음
        last = items[-1]
        ...
        if first and middle and last and first.isprintable() and middle.isprintable() and last.isprintable():
            # 정규 표현식을 사용하여 한글, 영어, 쉼표, 공백 외의 문자를 제거 (last에는 적용하지 않음)
            middle = re.sub('[^a-zA-Zㄱ-ㅣ가-힣, ]', '', middle)
            # 연속된 쉼표를 하나로 치환
...
# 결과를 파일에 저장
with open('/content/drive/MyDrive/BDP/ForeignData/2021_NIKE_F_new.txt', 'w') as f:
    for result in price_results_processed:
